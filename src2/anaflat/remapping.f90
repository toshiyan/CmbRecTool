...
subroutine remap_old(nn,D,vec,map,dif)
! not accurate interpolation...
  implicit none
  !I/O
  character(*), intent(in) :: dif
  integer, intent(in) :: nn(1:2)
  double precision, intent(in) :: D(1:2), vec(:,:)
  double precision, intent(inout) :: map(:)
  !internal
  integer :: i,j,a,b,npix,n
  double precision :: dx,dy,x0,y0
  double precision, allocatable :: imap(:,:), w(:,:), xi(:), yi(:)

  npix = nn(1)*nn(2)
  allocate(xi(npix),yi(npix))
  dx = D(1)/dble(nn(1))
  dy = D(2)/dble(nn(2))
  ...
  ! interpolation on regular grid
  allocate(imap(nn(1),nn(2)),w(nn(1),nn(2)));  imap=0d0;  w=0d0
  n = 0
  do a=1, nn(1)
    do b=1, nn(2)
      n = n + 1
      ...
      x0 = xi(a)+vec(n,1)
      y0 = yi(b)+vec(n,2)
      if (i<1.or.j<1.or.i>nn(1).or.j>nn(2)) cycle
      call interp(x0,y0,map(n),xi(i),yi(j),imap(i,j),w(i,j))
      if (i<=nn(1)-1)  call interp(x0,y0,map(n),xi(i+1),yi(j),imap(i+1,j),w(i+1,j))
      if (j<=nn(2)-1)  call interp(x0,y0,map(n),xi(i),yi(j+1),imap(i,j+1),w(i,j+1))
      if (i<=nn(1)-1.and.j<=nn(2)-1) call interp(x0,y0,map(n),xi(i+1),yi(j+1),imap(i+1,j+1),w(i+1,j+1))
      if (i<2.or.j<2.or.i>nn(1)-2.or.j>nn(2)-2) cycle
      call interp(x0,y0,map(n),xi(i+2),yi(j),imap(i+2,j),w(i+2,j))
      call interp(x0,y0,map(n),xi(i+2),yi(j+1),imap(i+2,j+1),w(i+2,j+1))
      call interp(x0,y0,map(n),xi(i-1),yi(j),imap(i-1,j),w(i-1,j))
      call interp(x0,y0,map(n),xi(i-1),yi(j+1),imap(i-1,j+1),w(i-1,j+1))
      call interp(x0,y0,map(n),xi(i),yi(j-1),imap(i,j-1),w(i,j-1))
      call interp(x0,y0,map(n),xi(i),yi(j+2),imap(i,j+2),w(i,j+2))
      call interp(x0,y0,map(n),xi(i+1),yi(j-1),imap(i+1,j-1),w(i+1,j-1))
      call interp(x0,y0,map(n),xi(i+1),yi(j+2),imap(i+1,j+2),w(i+1,j+2))
    end do
  end do
  ...
  if (dif=='dif') then
    do a=1, nn(1)
      do b=1, nn(2)
        if(w(a,b)/=0d0) map(n) = map(n) - imap(a,b)/w(a,b)
        n = n + 1
      end do
    end do
  else
    do a=1, nn(1)
      do b=1, nn(2)
        if(w(a,b)/=0d0) map(n) = imap(a,b)/w(a,b)
        if(w(a,b)==0d0) map(n) = 0d0
        n = n + 1
      end do
    end do
  end if
  deallocate(imap,w,xi,yi)

end subroutine remap_old


subroutine interp(xi,yi,zi,x,y,z,w)
  implicit none
  double precision, intent(in) :: xi, yi, zi, x, y
  double precision, intent(inout) :: z, w
  double precision :: d
