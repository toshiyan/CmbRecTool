...
#ifdef old
subroutine elarrays(nn,D,elx,ely,els)
!* Return elx, ely and absolute value of multipole in 2D
  implicit none
  !I/O
  integer, intent(in) :: nn(1:2)
  double precision, intent(in) :: D(1:2)
  double precision, intent(out), optional :: elx(:), ely(:), els(:)
  !internal
  integer :: i, j, n
  double precision :: lx, ly
  double precision, dimension(:), allocatable :: elx0, ely0, els0

  allocate(elx0(nn(1)*nn(2)),ely0(nn(1)*nn(2)),els0(nn(1)*nn(2)));  elx0=0d0;  ely0=0d0;  els0=0d0
  ...
  do i = 1, nn(1)
    ...
    do j = 1, nn(2)
      ...
      elx0(n) = lx
      ely0(n) = ly
      if (lx/=0d0.or.ly/=0d0) els0(n) = dsqrt(lx**2+ly**2)
      n = n + 1
    end do
  end do
...
function ei2phi(lx,ly)
  ...
  if(lx==0d0.and.ly==0d0) then
    ei2phi = 0d0
  else
    ei2phi = ((lx**2-ly**2) + iu*2d0*lx*ly)/(lx**2 + ly**2)
  end if

end function ei2phi

subroutine map_read_pol_RT(nn,mapfile,QU)
  implicit none
  !I/O
  ...
  !internal
  double precision :: readx, ready, inmap(3)
  integer :: i, j, n

  n = 1
  open(unit=20,file=trim(mapfile),status='old')
  do i = 1, nn(1)
    do j = 1, nn(2)
      read(20,*) readx, ready, inmap(1:3)
      QU(n,1) = inmap(2)
      QU(n,2) = inmap(3)
      n = n + 1
    end do
...
end subroutine map_read_pol_RT

subroutine select_mapfile_RT(n,mapfile,root)
  implicit none
  !I/O
  character(*), intent(in) :: root
  character(*), intent(out) :: mapfile
  integer, intent(in) :: n

  if(n<10) then
    !mapfile = trim(root)//'cmb_TEmap_realz0'//str(n)//'.txt'
  else
    !mapfile = trim(root)//'cmb_TEmap_realz'//str(n)//'.txt'
  end if

endsubroutine select_mapfile_RT

subroutine map_read_dble(nn,mapfile,T,P)
  implicit none
  !I/O
  character(*), intent(in) :: mapfile
  integer, intent(in) :: nn(2)
  ...
  do i = 1, nn(1)
    do j = 1, nn(2)
      if(present(P)) then 
        read(20,*) readx, ready, inmap(1), inmap(2), inmap(3) 
      else
        read(20,*) readx, ready, inmap(1)
      end if
      T(n) = cmplx(inmap(1))
      if(present(P)) P(n) = (inmap(2)+iu*inmap(3))
      n = n + 1
    end do
  end do
  close(20)

end subroutine map_read_dble

subroutine map_read_cmplx(nn,mapfile,T,P)
  implicit none
  !I/O
  character(*), intent(in) :: mapfile
  integer, intent(in) :: nn(2)
  complex(dlc), intent(out) :: T(:)
  complex(dlc), intent(out), optional :: P(:)
  !internal
  double precision :: readT(nn(1)*nn(2))

  if(present(P)) then
    call map_read_dble(nn,mapfile,readT,P)
  else 
    call map_read_dble(nn,mapfile,readT)
  end if
  T = readT

end subroutine map_read_cmplx
