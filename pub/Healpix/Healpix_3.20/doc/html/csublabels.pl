# LaTeX2HTML 2002-2-1 (1.71)
# Associate labels original text with physical files.


$key = q/csub:read_healpix_map/;
$external_labels{$key} = "$URL/" . q|csubnode11.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:write_healpix_map/;
$external_labels{$key} = "$URL/" . q|csubnode13.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:get_fits_size/;
$external_labels{$key} = "$URL/" . q|csubnode7.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:nside2npix/;
$external_labels{$key} = "$URL/" . q|csubnode9.htm|; 
$noresave{$key} = "$nosave";

$key = q/sub:subroutines/;
$external_labels{$key} = "$URL/" . q|csub.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:vec2ang/;
$external_labels{$key} = "$URL/" . q|csubnode12.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:npix2nside/;
$external_labels{$key} = "$URL/" . q|csubnode8.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:pix_tools/;
$external_labels{$key} = "$URL/" . q|csubnode10.htm|; 
$noresave{$key} = "$nosave";

$key = q/csub:ang2vec/;
$external_labels{$key} = "$URL/" . q|csubnode6.htm|; 
$noresave{$key} = "$nosave";

1;


# LaTeX2HTML 2002-2-1 (1.71)
# labels from external_latex_labels array.


1;

